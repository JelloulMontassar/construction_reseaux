        ...
        <div class="mb-3">
          ...
          <div class="card">
            ...
            <div class="card-body p-6 border-bottom mb-4">
              ...
              <!-- button -->
              <MazSlider v-model="ProjectDetails.cpercentage"  />
              <div class="d-flex justify-content-center">
                   <button  class="btn btn-outline-primary" @click="ChangeProgress(ProjectDetails.id,ProjectDetails.cpercentage)"
                ...
                <li class="list-group-item" v-else>
                  <div class="d-grid gap-2 col-12 mx-auto">
                    <button
                      ...
                      data-bs-toggle="modal"
                      data-bs-target="#assignTeam"
                      data-bs-dismiss="offcanvas"
                      aria-label="Close"
                      @click="getTeams(ProjectDetails.id)"
                    >
                      <i class="bi bi-info-circle"></i> Assign a team members
                    </button>
                  </div>
                </li>
                <li class="list-group-item">
...
export default {
  ...
  methods: {
    ...
    getProjects() {
      ProjectService.getMyProjects().then((reponse) => {
        this.Projects = reponse.data;
        console.log(this.Projects);
      });
    },
    AssignTeam() {
      ProjectService.AssignTeam(this.project_details_id, this.checked).then(
        (response) => {
          console.log(response.data);
        }
      );
    },
    ChangeProgress(project_id,percentage){
      ProjectService.UpdateProgress(project_id,percentage).then(response=>{
        console.log(response.data)
      })
    },
